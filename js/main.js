(() => {
  ...
  document.addEventListener('contextmenu', e => {
    if (e.target.tagName === 'IMG' || e.target.closest('.item, .cat-card, .search-item, .print-card')) {
      e.preventDefault();
    }
  });
  ...
  const lightboxBuy = lightbox ? lightbox.querySelector('.lightbox-buy') : null;

  document.querySelectorAll('[data-lightbox]').forEach(el => {
    el.addEventListener('click', () => {
      if (!lightbox || !lightboxImg) return;
      lightboxImg.src = el.dataset.lightbox;
      const title = el.dataset.title || el.querySelector('.item-label')?.textContent || '';
      if (lightboxTitle) lightboxTitle.textContent = title;
      if (lightboxBuy) {
        lightboxBuy.href = 'contact.html?image=' + encodeURIComponent(title) + '&subject=Print Purchase Inquiry';
      }
